...
export function handles() {
  ...
  const jobTitle: StepFactory = () => {
    ...
    const globalBlacklist = [
      '销售', '电销', '房产经纪', '经纪人', '电话销售', '面销', '地推', '门市', '客服', '售后服务', '售前咨询', 
      '司机', '代驾', '货运', '客运', '送货', '骑手', '送外卖', '快递员', '课程顾问', '课顾', 
      '教师', '老师', '助教', '前台', '文员', '收银', '导购', '营业员', '主播', '保安', '保洁', '普工', 
      '操作工', '质检员', '车间', '服务员', '前厅', '后厨', '配菜', '洗碗', '保姆', '育婴', '月嫂'
    ]
    return async ({ data }, _ctx) => {
      try {
        ...
        // 1. 强力底层黑名单拦截，直接过滤非互联网、销售、司机、服务员等无关岗位
        for (const item of globalBlacklist) {
          if (text.includes(item)) {
            throw new JobTitleError(`匹配到全局不相关岗位黑名单: ${item}`)
          }
        }
